...
Use `flow.executors` to limit which built-in flow executors the server registers. This setting applies only to built-in executors. When you embed <ProductName /> and register custom executors, use **Flow.RegisterCustom** in `EngineConfig` (see `pkg/thunderidengine/README.md`); `flow.executors` does not affect custom executors.
...
**Available built-in executor names:**
...
:::warning
Every flow you deploy must reference only executors that are registered on the server. If a flow uses an executor that is not in your `flow.executors` list (and you have not registered a custom executor with the same name), flow validation fails at startup.
:::
