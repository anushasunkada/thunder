...
// Package appmgmt defines the data structures for the appmgmt module.
//
//nolint:lll
package appmgmt
...
type ApplicationCertificate struct {
	Type  CertificateType `json:"type,omitempty" yaml:"type,omitempty" jsonschema:"Certificate type. Specifies the certificate format (e.g., PEM, JWK). Used for certificate-based client authentication or JWT signature validation."`
...
type OAuthAppConfigComplete struct {
	...
	GrantTypes              []GrantType             `json:"grantTypes" yaml:"grant_types"`
	ResponseTypes           []ResponseType          `json:"responseTypes" yaml:"response_types"`
	TokenEndpointAuthMethod TokenEndpointAuthMethod `json:"tokenEndpointAuthMethod" yaml:"token_endpoint_auth_method"`
...
type OAuthAppConfigDTO struct {
	...
	GrantTypes              []GrantType             `json:"grantTypes,omitempty" jsonschema:"OAuth grant types. Common: [authorization_code, refresh_token] for user apps, [client_credentials] for M2M."`
	ResponseTypes           []ResponseType          `json:"responseTypes,omitempty" jsonschema:"OAuth response types. Common: [code] for user apps. Omit for M2M."`
	TokenEndpointAuthMethod TokenEndpointAuthMethod `json:"tokenEndpointAuthMethod,omitempty" jsonschema:"Client authentication method. Use 'none' for Public clients, 'client_secret_basic' for Confidential/M2M."`
...
// ApplicationListOutput represents the output for list_applications MCP tool.
