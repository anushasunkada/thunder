...
// greetExecutor is a custom flow executor. It embeds a base built with NewBaseExecutor so it
...
func Example() {
	...
	// Seed configuration (GateClient, OAuth, flow, crypto, declarative resources, ...) the same
	// way the standalone server does, without importing internal/* config packages.
	cfg, err := thunderidengine.LoadConfig(serverHome)
	if err != nil {
		log.Fatal(err)
	}
	...
	eng, err := thunderidengine.New(
		...
		// Let the engine build the executor registry from these dependencies. Inject executor
		// extras here too (for example a custom ConsentEnforcer when enabling ConsentExecutor).
		...
		// Every system-of-record service left uninjected (OU, resource, IDP, authz, attribute
		// cache, design, flow, i18n) falls back to declarative file-based resources.
	)
	if err != nil {
		log.Fatal(err)
	}
