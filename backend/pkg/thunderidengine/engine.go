...
func New(opts ...Option) (*Engine, error) {
	...
	// Build the declarative default i18n service first; the declarative application service
	// (built in the base graph) depends on it.
	...
	// Build the declarative system-of-record services the embedder did not inject. This records
	// ou/resource/idp/authz/attributeCache on the config; flow provider and design are finished
	// after the executor registry is built (they depend on it).
	var declBase *declarativeBase
	if needDeclarativeBase {
		b, err := c.buildDeclarativeBaseServices(cacheManager, c.jwtService)
		if err != nil {
			return nil, err
		}
		declBase = b
	...
	if err := c.finalizeExecutorsAndFlow(declBase, cacheManager, flowFactory, graphCache, declarative); err != nil {
		return nil, err
	}
...
func (c *engineConfig) finalizeExecutorsAndFlow(
	...
	// Finish the declarative graph (flow provider + design resolve) now the registry exists.
	if declBase != nil {
		if err := c.buildDeclarativeFlowAndDesign(
			declBase, cacheManager, flowFactory, graphCache, c.executorRegistry, c.i18nService); err != nil {
			return err
		}
	} else if c.flowProvider == nil && declarative && c.executorRegistry != nil {
		fp, err := buildDefaultFlowProvider(cacheManager, flowFactory, graphCache, c.executorRegistry)
		if err != nil {
			return err
		}
		c.flowProvider = fp
...
func (e *Engine) RegisterRoutes(mux *http.ServeMux) error {
	...
	flowExecService, err := flowexec.Initialize(
		mux, e.cfg.flowProvider, e.cfg.actorProvider, e.cfg.executorRegistry,
