...
// Package clientprovidertest provides testify mocks for thunderidengine.ClientProvider.
//
//nolint:revive // test-only mock; exported methods mirror the interface without doc comments.
package clientprovidertest
...
// ClientProviderMock is a testify mock for thunderidengine.ClientProvider.
type ClientProviderMock struct {
	mock.Mock
}

// NewClientProviderMock creates a ClientProviderMock registered with t.
func NewClientProviderMock(t interface {
	mock.TestingT
	Cleanup(func())
}) *ClientProviderMock {
	m := &ClientProviderMock{}
	m.Mock.Test(t)
