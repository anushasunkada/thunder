...
// InitTestFromServerRuntime reloads OAuth2 config from the initialized server runtime.
// For use in tests after config.InitializeServerRuntime.
func InitTestFromServerRuntime() {
	Reset()
	rt := sysconfig.GetServerRuntime()
	Set(FromSystemConfig(rt.Config, BuildOptionsForServer(&rt.Config)))
}

// InitTestServerRuntime initializes server runtime and OAuth2 config for tests.
