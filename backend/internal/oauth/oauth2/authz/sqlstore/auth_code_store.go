...
// Package sqlstore provides the relational-database-backed authorization code and
// authorization request stores. It is isolated from the authz package so that
// consumers needing only the interfaces (or the Redis implementations) do not
// transitively link the SQL database drivers.
package sqlstore
...
// authorizationCodeStore implements authz.AuthorizationCodeStoreInterface for managing authorization codes.
type authorizationCodeStore struct {
	dbProvider   provider.DBProviderInterface
	deploymentID string
}

// NewAuthorizationCodeStore creates a new database-backed authorization code store.
func NewAuthorizationCodeStore(deploymentID string) authz.AuthorizationCodeStoreInterface {
...
func (acs *authorizationCodeStore) InsertAuthorizationCode(
	ctx context.Context, authzCode authz.AuthorizationCode) error {
...
func (acs *authorizationCodeStore) ConsumeAuthorizationCode(ctx context.Context, authCode string) (bool, error) {
	...
	rowsAffected, err := dbClient.ExecuteContext(ctx, queryConsumeAuthorizationCode,
		authz.AuthCodeStateInactive, authCode, authz.AuthCodeStateActive, acs.deploymentID)
	if err != nil {
...
func (acs *authorizationCodeStore) GetAuthorizationCode(
	...
	if len(results) == 0 {
		return nil, authz.ErrAuthorizationCodeNotFound
	}
	row := results[0]
...
func (acs *authorizationCodeStore) getJSONDataBytes(authzCode authz.AuthorizationCode) ([]byte, error) {
...
// buildAuthorizationCodeFromResultRow builds an authz.AuthorizationCode from a database result row.
func buildAuthorizationCodeFromResultRow(row map[string]interface{}) (*authz.AuthorizationCode, error) {
...
func appendAuthzDataJSON(
	row map[string]interface{}, authzCode *authz.AuthorizationCode,
) (*authz.AuthorizationCode, error) {
	var dataJSON string
