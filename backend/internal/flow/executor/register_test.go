...
func (suite *BuiltInExecutorRegistrationTestSuite) depsForBuiltInRegistration() RegisterDeps {
	return RegisterDeps{
...
func (suite *BuiltInExecutorRegistrationTestSuite) TestRegisterBuiltInExecutor_EachDefaultNameRegisters() {
	...
	for _, name := range defaultBuiltInExecutorNames() {
		...
		err := registerBuiltInExecutor(reg, deps, name)
...
func (suite *BuiltInExecutorRegistrationTestSuite) TestRegisterBuiltInExecutor_RequiresFlowFactory() {
	...
	suite.Require().Panics(func() {
		_ = registerBuiltInExecutor(reg, RegisterDeps{}, ExecutorNameInviteExecutor)
	})
...
func (suite *BuiltInExecutorRegistrationTestSuite) TestRegisterBuiltInExecutors_UnknownName() {
	err := registerBuiltInExecutors(suite.registry, RegisterDeps{}, []string{"NotARealExecutor"})
...
func (suite *BuiltInExecutorRegistrationTestSuite) TestResolveBuiltInExecutorNames_WhitespaceNameRejected() {
	_, err := resolveBuiltInExecutorNames([]string{" " + ExecutorNameBasicAuth})
	require.Error(suite.T(), err)
	assert.Contains(suite.T(), err.Error(), "unknown built-in executor")
}
...
func (suite *BuiltInExecutorRegistrationTestSuite) TestInitialize_UnknownExecutorReturnsError() {
	reg, err := Initialize(RegisterDeps{}, config.FlowConfig{
...
func (suite *BuiltInExecutorRegistrationTestSuite) TestRegisterBuiltInExecutor_RegistryNilExecutorSkipFails() {
	...
	err := registerBuiltInExecutor(reg, suite.depsForBuiltInRegistration(), ExecutorNamePermissionValidator)
	require.Error(suite.T(), err)
...
func (suite *BuiltInExecutorRegistrationTestSuite) TestRegisterBuiltInExecutor_RegistrarErrorPropagates() {
	const testName = ExecutorNamePermissionValidator
	original := builtInExecutorRegistrars[testName]
	suite.T().Cleanup(func() {
		builtInExecutorRegistrars[testName] = original
	})
	builtInExecutorRegistrars[testName] = func(ExecutorRegistryInterface, RegisterDeps) error {
		return fmt.Errorf("registration failed")
	}

	reg := newExecutorRegistry()
	err := registerBuiltInExecutor(reg, suite.depsForBuiltInRegistration(), testName)
	require.Error(suite.T(), err)
	assert.Contains(suite.T(), err.Error(), "registration failed")
	assert.False(suite.T(), reg.IsRegistered(testName))
}
