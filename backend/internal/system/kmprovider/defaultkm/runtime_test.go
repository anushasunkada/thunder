...
func asRuntimeSvc(provider thunderidengine.RuntimeCryptoProvider) *runtimeCryptoService {
	return provider.(*runtimeCryptoService)
}
...
func TestEncrypt_RSAOAEP256_SuccessViaConstructor(t *testing.T) {
	...
	svc := asRuntimeSvc(NewRuntimeCryptoService(pkiMock, nil))
	...
	wrappedCEK, details, err := svc.encrypt(
		context.Background(), &thunderidengine.KeyRef{KeyID: testKeyID}, params, nil,
	)
	require.NoError(t, err)
...
func TestEncrypt_RSAOAEP256_GetPublicKeyError(t *testing.T) {
	...
	_, _, err := svc.encrypt(context.Background(), &thunderidengine.KeyRef{KeyID: testKeyID}, params, []byte("data"))
	assert.Error(t, err)
...
func TestEncrypt_ECDHES_Success(t *testing.T) {
	...
	_, details, err := svc.encrypt(context.Background(), &thunderidengine.KeyRef{KeyID: testKeyID}, params, nil)
	require.NoError(t, err)
	assert.NotNil(t, details)
}
...
func TestEncrypt_ECDHES_GetPublicKeyError(t *testing.T) {
	...
	_, _, err := svc.encrypt(context.Background(), &thunderidengine.KeyRef{KeyID: testKeyID}, params, nil)
	assert.Error(t, err)
	assert.Contains(t, err.Error(), testKeyID)
}

func TestEncrypt_UnsupportedAlgorithm(t *testing.T) {
	svc := asRuntimeSvc(NewRuntimeCryptoService(nil, nil))
...
func TestEncrypt_RSAOAEP256_PKIError(t *testing.T) {
	...
	keyRef := &thunderidengine.KeyRef{KeyID: "key1"}
	params := cryptolab.AlgorithmParams{
		Algorithm:  cryptolab.AlgorithmRSAOAEP256,
		RSAOAEP256: cryptolab.RSAOAEP256Params{ContentEncryptionAlgorithm: "A256GCM"},
	}
...
func TestGetPublicKeys_NilPKIService(t *testing.T) {
	...
	_, err := svc.GetPublicKeys(context.Background(), thunderidengine.PublicKeyFilter{})
...
func TestGetPublicKeys_RSA(t *testing.T) {
	...
	svc := &runtimeCryptoService{pkiService: pki, logger: newTestLogger()}
	keys, err := svc.GetPublicKeys(context.Background(), thunderidengine.PublicKeyFilter{})
	require.NoError(t, err)
	require.Len(t, keys, 1)
	assert.Equal(t, "key1", keys[0].KeyID)
	assert.Equal(t, thunderidengine.AlgorithmRS256, keys[0].Algorithm)
...
func TestGetPublicKeys_EdDSA(t *testing.T) {
	...
	require.NoError(t, err)
	require.Len(t, keys, 1)
	assert.Equal(t, thunderidengine.AlgorithmEdDSA, keys[0].Algorithm)
}
...
func TestGetPublicKeys_FilterByKeyID(t *testing.T) {
	...
	svc := &runtimeCryptoService{pkiService: pki, logger: newTestLogger()}
	keys, err := svc.GetPublicKeys(context.Background(), thunderidengine.PublicKeyFilter{KeyID: "rsa-key"})
	require.NoError(t, err)
	require.Len(t, keys, 1)
...
func TestGetPublicKeys_FilterByAlgorithm(t *testing.T) {
	...
	keys, err := svc.GetPublicKeys(context.Background(),
		thunderidengine.PublicKeyFilter{Algorithm: cryptolab.AlgorithmES256})
	require.NoError(t, err)
	require.Len(t, keys, 1)
